...
void BVH::Divide(size_t n, std::vector<AccelNode> *children) {
  ...
  switch (mode) {
    ...
    case BVHMode::BVH:
    case BVHMode::SAH: {
      DivideBySAH(&node, &node_left, &node_right);
      break;
    }
  }
...
void BVH::DivideBySAH(AccelNode *node, AccelNode *node_left, AccelNode *node_right) {
  //在最长维度排序
  ...
  for (size_t i = 1; i < kNumBuckets; i++) {
    ...
    auto mid = node->faces_index.begin() + static_cast<int>((node->faces_index.size()) * i / kNumBuckets);
